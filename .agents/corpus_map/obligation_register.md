...
| `OBL-CMSA-001` | Completed Log-Derivative Pointwise Decomposition | Prove $P(u) = A(u) - \Xi'/\Xi(u-1/2)$ on $\Re(u) > 1$ | Lean 4 `ConditionalCompletedLogDerivativeDecomposition` | `PROVED / EXACT` |
...
| `OBL-CMSA-003` | Spectral Resolvent Mean-Square Regularization (Gate G4) | Regularize translation-average integral $(1/2T)\int_{-T}^T$ on $L^2$ zero resolvents to extract discrete positive radial curvature | Identified earliest open obstruction (Gate G4) | `OPEN (GATE G4)` |
...
| `OBL-CMSA-005` | Complete Finite Spectral Expansion Closure | Prove and verify exact closed-form finite expansion $S_{N,T}(\sigma) = I_{AA} - I_{AZ} - I_{ZA} + I_{ZZ}$ via kernels $J_T, K_T$ | Arbitrary-precision verification in `math_core.py` ($< 10^{-18}$) | `PROVED / VERIFIED` |
