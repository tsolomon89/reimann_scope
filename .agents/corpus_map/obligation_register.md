...
| `OBL-CT-001A` | Non-Scalar Arithmetic Functional Construction | Construct a zero-independent, non-scalar arithmetic functional \(\mathscr A_\tau(\xi)\) from prime powers and archimedean factors, specifying convergence domains and certified regularization without supplying zero inputs | Mathematical definition & regularization (`CURVATURE_TRANSPORT.md` §15) | `OPEN (CANONICAL EARLIEST OBLIGATION)` |
