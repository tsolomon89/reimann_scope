...
| `OBL-CMSA-003` | Spectral Resolvent Mean-Square Regularization (Gate G4) | Gate G4 is the earliest open gate in the present CMSA derivation: regularize translation-average integral $(1/2T)\int_{-T}^T$ on $L^2$ zero resolvents to extract discrete positive radial curvature | Analysis / Regularization barrier (Gate G4) | `OPEN (GATE G4)` |
