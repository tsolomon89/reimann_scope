...
| `OBL-CMSA-003` | Spectral Resolvent Mean-Square Regularization (Gate G4) | Gate G4 is the earliest open gate in the present CMSA derivation: regularize translation-average integral $(1/2T)\int_{-T}^T$ on $L^2$ zero resolvents to extract discrete positive radial curvature; raw finite candidate & additive class classified as `INCONCLUSIVE_WITH_PRECISE_EARLIEST_OPEN_SUBGATE`; earliest open subgate: prove a negative raw response analytically or with validated outward-rounded interval arithmetic on general infinite/regularized limit | Analysis / Regularization barrier (Gate G4) | `OPEN (GATE G4)` |
