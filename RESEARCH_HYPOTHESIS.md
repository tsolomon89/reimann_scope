...
1. **Regime 4A (Dense stations and negative-grade continuum limit)**:
   ...
   - **Shared-Grade Rigidity and Retraction of Unrestricted 1D Barrier**: For bounded coefficients ($\|c\| \le B$), single-grade combinations at fixed grades collapse to a 1-dimensional subspace spanned by $F_{\infty,0,w}$, yielding an empirical error plateau against independent smooth targets $f_* \ne \lambda F_{\infty,0,w}$. However, the prior claim that varying families over changing grades and bandwidths collapse to a 1D subspace under unrestricted coefficients is **retracted**: column convergence $F_j \to v$ in $H^1$ does not imply that varying spans have limiting dimension 1, since difference quotients across grades $(F_j - F_k)/(\epsilon_j - \epsilon_k)$ isolate transverse directions. Whether the unrestricted closure of varying spans has a non-trivial closure gap or is dense in $\mathcal V_R$ remains an **open mathematical question**. In numerical least-squares optimization, cancelling the common continuum mode produces severe ill-conditioning, and the critical open research question is: *after cancelling the common continuum profile $F_{\infty,0,w}$ between actual TC grades, what arithmetic directions survive, and can those directions supply the detection step your contradiction requires?*
