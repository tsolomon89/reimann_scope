...
1. **Regime 4A (Dense stations and negative-grade continuum limit)**:
   ...
   - **Shared-Grade Rigidity and Exact Restricted Bounded-Coefficient Lemma**:
     For finite grade sets $C_j$, normalized coefficients $b_{j,K} = c_{j,K}/a_K$, and limiting profile $v = F_{\infty,0,w}$, the elementary triangle inequality establishes:
     ...
     When normalized coefficient sums are uniformly bounded ($\sum_K |b_{j,K}| \le B$), uniform column convergence $\|F_{K,h_j,w} - v\|_{H^1} \to 0$ implies that the distance to $\operatorname{span}\{v\}$ tends to zero ($B \cdot \epsilon \to 0$). However, distance to a line tending to zero does not assert that the combinations themselves converge, nor does it establish a universal 99% error or an irreducible closure gap for the unrestricted family: targets outside the line can be arbitrarily close to it, and varying spans under unrestricted coefficients form difference quotients $(F_j - F_k)/(\epsilon_j - \epsilon_k)$ isolating transverse directions. Whether the unrestricted closure of varying spans is dense in $\mathcal V_R$ or has a non-trivial closure gap remains an **open mathematical question**. In numerical least-squares optimization, cancelling the common continuum mode produces ill-conditioned systems, and the crucial open research question is: *after cancelling the common continuum profile $F_{\infty,0,w}$ between actual TC grades, what arithmetic directions survive, and can those directions supply the detection step your contradiction requires?*
   - **Existential Diagonal Convergence Schedule**:
     The establishment of the fixed-$h$ weak PNT limit $F_{K,h,w} \xrightarrow{K \to -\infty} F_{\infty,h,w}$ and the continuum smoothing limit $F_{\infty,h,w} \xrightarrow{h \to 0} F_{\infty,0,w} =: v$ in $H^1$ analytically proves the **existence of a convergent diagonal schedule**: for any $j \ge 1$, first choose $h_j \downarrow 0$ such that $\|F_{\infty,h_j,w} - v\|_{H^1} < 1/(2j)$, then choose $K_j < \min(K_{j-1}, -j)$ sufficiently negative such that $\|F_{K_j,h_j,w} - F_{\infty,h_j,w}\|_{H^1} < 1/(2j)$. By the triangle inequality, $\|F_{K_j,h_j,w} - v\|_{H^1} < 1/j \to 0$. This existence theorem is distinct from providing an effective rate, an affordable finite grid, or validating an arbitrary shallow schedule.
   - **Positivity & Prime Resonance on Dense Negative Grades**:
     Ordinary support overlap does not by itself establish prime resonance or loss of positive definiteness. In dense negative grades with overlapping bumps, when prime-power resonance gaps $\Delta_{\rm res} < 2h$, non-zero prime terms $W_{\rm prime}$ are activated. However, their sign and magnitude depend on the explicit arithmetic weights and the full quadratic form sum. Positivity on dense negative grades is neither proved nor disproved; it remains uncertified and untested.
...
3. **Joint Diagonal Schedule and Numerical Precision Caveat**:
   Along the tested shallow diagonal schedule $[(0, 0.20), (-1, 0.10), (-2, 0.05), (-3, 0.02)]$, total relative $H^1$ error increased monotonically ($3.39 \to 19.79 \to 102.67 \to 1083.34$ on a 601-point grid, shifting to $\sim 1063.78$ upon grid refinement). This observed worsening confirms empirical divergence along this specific shallow schedule. However, these reported values are mesh-dependent floating-point quadrature approximations, not certified Arb ball enclosures. Furthermore, empirical divergence on a shallow schedule does not establish a theoretical obstruction to joint convergence for deeper schedules, nor does it establish a necessary bandwidth law ($E_K \ll h^{7/2}$).
