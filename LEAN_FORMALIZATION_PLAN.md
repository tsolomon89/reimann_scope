...
4. **Mathlib `Filter.Tendsto` Theorems & Cofinal Sequence Limit Countermodels** (`tendsto_cofinal_fixed_zero`, `not_tendsto_cofinal_diagonal_zero`, `finite_sum_tendsto_interchange`, `cofinal_sequence_fixed_limit_zero`, `cofinal_diagonal_not_tendsto_zero`, `cofinal_sequence_diagonal_witness`, `cofinal_schedule_distinct_from_fixed_limit`):
   - Mathlib Tendsto fixed-\(H\) limit: `Tendsto (fun (n : ℕ) => H / ((n : ℝ) + 1)) atTop (𝓝 0)`.
   - Mathlib Tendsto diagonal non-convergence: `¬ Tendsto (fun (n : ℕ) => ((n : ℝ) + 1) / ((n : ℝ) + 1)) atTop (𝓝 0)`.
   - Mathlib Tendsto finite-sum limit interchange: `Tendsto (fun n => ∑ i in s, f i n) atTop (𝓝 (∑ i in s, g i))`.
   - Elementary \(\varepsilon\)-\(N\) fixed-\(H\) limit theorem: \(\forall H \in \mathbb R, \forall \varepsilon > 0, \exists N, \forall n \ge N, |H / (n + 1)| < \varepsilon\).
   - Elementary \(\varepsilon\)-\(N\) diagonal non-convergence: \(\neg (\forall \varepsilon > 0, \exists N, \forall n \ge N, |(n+1)/(n+1) - 0| < \varepsilon)\).
