...
| G4-023 | Live Cofinal Perturbation Decomposition & Norm-Growth Threshold: response decomposes into direct energy and arithmetic cross-term $V_T = E_T - C_T$ where $E_T = \|\Delta_{H(T)}\|^2/(2T)$ and $C_T = (1/T)\Re\langle P_\sigma, \Delta_{H(T)}\rangle$. Supercritical growth $\|\Delta_{H(T)}\| \ge c\sqrt{T}$ is necessary but not sufficient for positive response due to possible cross-term cancellation; isolating the exact asymptotic growth and sign of $E_T$ and $C_T$ along $H(T) = cT$ is the live open obligation `OBL-CMSA-003-G4-COFINAL-ESTIMATE`. | **THEORETICAL ANALYSIS / OPEN OBLIGATION** | Documented in `CMSA_GATE_G4.md` §9.8 and `MATH_CONTRACT.md` §42.11; Gate G4 remains open. |
