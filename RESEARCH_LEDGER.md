...
| ID | Statement | Classification | Reason |
|---|---|---|---|
| INT-001 | Exact Quartet Resolvent Difference Identity: $\Delta Z_+(z) = \frac{1}{z-(\delta+i\gamma)} + \frac{1}{z-(-\delta+i\gamma)} - \frac{2}{z-i\gamma} = \frac{2\delta^2}{(z-i\gamma)((z-i\gamma)^2-\delta^2)}$. | **PROVED / FORMALLY_PROVED** | Formalized in Lean 4 (`exact_quartet_resolvent_identity`, `ALGEBRAIC_IDENTITY`). |
...
| INT-003 | CMSA–RDQ Logarithmic Derivative Connection: $\frac{\partial}{\partial z}\log q_{\delta,\gamma}(z) = \Delta Z_+(z)$. | **PROVED / EXACT** | Exact rational derivative identity verified in `evaluate_cmsa_rdq_derivative_identity`. |
| INT-004 | Exact Fourier–Laplace Transform: $\widehat{\Delta Z_\sigma}(\xi) = 8\pi e^{-a\xi}(\cosh(\delta\xi)-1)\cos(\gamma\xi)$ for $\xi > 0$. | **PROVED / EXACT** | Proved analytically and verified in `evaluate_fourier_quartet_difference`. |
| INT-005 | Prime Cross-Term and Unnormalized Limit Loss: $-2\Re\int_{\sigma_0}^\infty \int_{\mathbb R} P_\sigma \overline{\Delta Z_\sigma} dt d\sigma = -4\pi \sum \Lambda(n)\frac{n^{1/2-2\sigma_0}}{\log n}(\cosh(\delta\log n)-1)\cos(\gamma\log n)$, sign-indefinite; unnormalized integration diverges on base prime diagonal ($\int |P_\sigma|^2 = \infty$), losing arithmetic anchor (`FAIL_ZERO_ARITHMETIC_ANCHOR_UNDER_UNNORMALIZED_LIMIT`). | **PROVED / CLOSED BRANCH** | Documented in `CMSA_GATE_G4.md` §9.12 and `test_bilateral_second_variation.py`. |
...
| BG-003 | Bilateral Second-Order Asymmetry Cross-Term No-Go: under coordinate dilation $\Delta_{\pm h}(z) = \Delta Z(\tau^{\pm h}z)$, $\Delta_h + \Delta_{-h} = 2h^2 B(z) + \mathcal O(h^4) \ne 0$, leaving uncancelled background cross-term $4h^2\Re(F\bar B) \ne 0$ (`FAIL_BILATERAL_CROSS_TERM_CANCELLATION`). | **PROVED / FORMALLY_PROVED** | Formalized in Lean 4 (`bilateral_second_order_asymmetry_cross_term`, `NO_GO_COMPONENT`). |
