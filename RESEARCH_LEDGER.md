...
| CT-007 | Grade character modulus $|\chi_\rho(K)| = \exp(K\delta\log\tau)$ and reflection reciprocal modulus $|\chi_{\rho^\#}(K)| = |\chi_\rho(K)|^{-1}$. | **PROVED / FORMALLY_PROVED** | Formalized in Lean 4 (`grade_character_modulus_def`, `reflection_reciprocal_modulus_prod`). |
| CT-008 | Reflection-pair defect $B_\rho(K) = 2(\cosh(K\delta\log\tau)-1) = 4\sinh^2(K\delta\log\tau/2) \ge 0$, with $B_\rho(K) = 0 \iff \delta = 0$ for $K \ne 0$. | **PROVED / FORMALLY_PROVED** | Formalized in Lean 4 (`reflection_pair_defect_cosh`, `reflection_pair_defect_nonneg`, `reflection_pair_defect_eq_zero_iff`). |
| CT-009 | Native grade curvature identity $B_\rho''(0) = 2\delta^2(\log\tau)^2$ yields normalized transport curvature $\mathscr K_\tau(\rho) = B_\rho''(0)/(2(\log\tau)^2) = \delta^2$. | **PROVED / FORMALLY_PROVED** | Formalized in Lean 4 (`native_grade_second_order_taylor_coefficient`). |
...
| CT-011 | Theta–Mellin scaling law: $a^{1/2}\int_0^\infty \Theta_a^+(t) t^{s/2-1} dt = a^{1/2-s}\pi^{-s/2}\Gamma(s/2)\zeta(s)$ matches scale factor $\tau^{-K(s-1/2)} = \chi_s(K)^{-1}$ at $a = \tau^K$. | **PROVED / EXACT** | Proved analytically and verified numerically in `verify_theta_mellin_scaling`. |
| CT-012 | Candidate CT-1 (Scalar Theta–Mellin Transport) vanishing obstruction: at any zero $\rho$ with $\Lambda(\rho)=0$, $F_K(\rho) = \tau^{-K(\rho-1/2)}\Lambda(\rho) \equiv 0$, so all direct $K$-derivatives vanish identically ($0 \equiv 0$) failing to detect $\delta \ne 0$; classified as `GRADE_COORDINATE_REDUNDANT` / `FAIL_ARITHMETIC_FIREWALL`. | **PROVED / FALSIFIED** | Verified in `verify_scalar_zero_multiplication_obstruction`. |
| CT-013 | Symmetry-complete polynomial countermodel $P_{\delta,\gamma}(z) = ((z-i\gamma)^2-\delta^2)((z+i\gamma)^2-\delta^2)$ satisfies even symmetry, Schwarz reflection, and positive curvature for $\delta \ne 0$, proving geometry + symmetry + curvature detection $\ne$ RH without arithmetic input. | **PROVED / FORMALLY_PROVED COUNTERMODEL** | Formalized in Lean 4 (`countermodelPolynomial_even`). |
...
| CT-015 | Arithmetic Curvature-Descent Theorem (`OBL-CT-001`): constructing a divisor-independent arithmetic functional $\mathscr A_\tau(\xi) = 0$ whose spectral expansion is $\sum W_\rho \delta_\rho^2$ is the program's canonical open obligation, superseding raw CMSA $1/(2T)$ activation. | **CANONICAL OPEN OBLIGATION** | Formulated in `CURVATURE_TRANSPORT.md` §12. |
