...
| G4-018 | Exact Remainder Cancellation Theorem: $R = F - Z \implies Z + R = F$, proving that $S_T(Z_H; R_H) = \frac{1}{2\pi}\int W_T |Z_H + R_H|^2 dt \equiv \frac{1}{2\pi}\int W_T |\Xi'/\Xi|^2 dt$ is identically independent of $H$ and $H(T)$, collapsing to full completed function evaluation. | **PROVED / FORMALLY_PROVED** | Formalized in Lean 4 (`exact_remainder_cancellation`, `functional_decomposition_independence`); candidate classified as `COLLAPSED_COFINAL_IDENTITY` / `FAIL_RADIAL_POSITIVITY`. |
