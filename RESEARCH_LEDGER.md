...
| G4-006 | Raw finite-window radial response $\Delta S_W$ achieves certified negative values on compact Fejér WIT-02 and negative numerical evidence on WIT 1, 3, 4. | **CERTIFIED (WIT-02) / EVIDENCE (WIT 1,3,4)** | Enclosed in outward-rounded Arb ball arithmetic for WIT-02; classified as `INCONCLUSIVE_WITH_PRECISE_EARLIEST_OPEN_SUBGATE` on general parameters. |
...
| G4-009 | Second-order radial response coefficient $C_W(\sigma, \gamma, T) = -2\Re \int W_T(t) F_0(t) \overline{D_\gamma(z)} dt$ governs leading variation $\Delta S_W = \delta^2 C_W + O(\delta^4)$ under uniform domination hypotheses. | **PROVED / EXACT** | Proved analytically and verified numerically in `evaluate_g4_radial_response_coefficient`. |
...
| G4-011 | Elementary absolutely convergent Dirichlet-series mean-square lemma for $\sum |a_n| < \infty$. | **PROVED / EXACT** | 4-step proof documented in `CMSA_GATE_G4.md` §4 without external black-box theorems. |
...
| G4-014 | Hypotheses for integrated pointwise delta expansions (window integrability, denominator separation, uniform domination, dominated convergence). | **PROVED / ANALYTIC BOUNDARY** | Documented in `MATH_CONTRACT.md` §42.8 and `CMSA_GATE_G4.md` §6. |
